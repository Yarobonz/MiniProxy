...
if (version_compare(PHP_VERSION, "5.4.7", "<")) {
  die("AnySchool requires PHP version 5.4.7 or later.");
}

$requiredExtensions = ["curl", "mbstring", "xml"];
foreach($requiredExtensions as $requiredExtension) {
  if (!extension_loaded($requiredExtension)) {
    die("AnySchool requires PHP's \"" . $requiredExtension . "\" extension. Please install/enable it on your server and try again.");
  }
}
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (X11; CrOS x86_64 14526.89.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.133 Safari/537.36";
  }
  $ch = curl_init();
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      //If the AnySchoolFormAction field appears in the POST data, remove it so the destination server doesn't receive it.
      $postData = [];
      parse_str(file_get_contents("php://input"), $postData);
      if (isset($postData["AnySchoolFormAction"])) {
        unset($postData["AnySchoolFormAction"]);
      }
  ...
  return ["headers" => $responseHeaders, "body" => $responseBody, "responseInfo" => $responseInfo]; //IMPORTANT
}
...
if (isset($_POST["AnySchoolFormAction"])) {
  ...
  //If the AnySchoolFormAction field appears in the query string, make $url start with its value, and rebuild the the query string without it.
...
if (empty($url)) {
    if (empty($startURL)) {
     ...
     input[type=submit]{
      ...
      <form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + btoa(document.getElementById('site').value); return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\">
      
          <input id=\"site\" type=\"text\" size=\"50\" placeholder=\"URL\" /><input type=\"submit\" value=\"Unblock\" />
...
if (empty($scheme)) {
  if (strpos($url, "//") === 0) {
    ...
    die('Error: Detected a "' . $scheme . '" URL. AnySchool exclusively supports http[s] URLs.');
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="AnySchoolFormAction" value="' . htmlspecialchars($action) . '" />');
